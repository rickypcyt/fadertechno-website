...
export default async function EventPage({
  ...
  const eventTime = new Date(event.startDate).toLocaleTimeString(localeStr, {
    hour: '2-digit',
    minute: '2-digit',
  })
        ...
        <div className="event-content">
          <div className="event-header">
            <div className="event-cover-date">
              <span className="event-cover-day">
                {new Date(event.startDate).toLocaleDateString(localeStr, { day: '2-digit' })}
              </span>
              <span className="event-cover-month">
                {new Date(event.startDate).toLocaleDateString(localeStr, { month: 'short' }).toUpperCase()}
              </span>
            </div>
          ...
          {event.artists.length > 0 && (
            <div className="event-section">
              <div className="event-section-label">{dict.event.lineup}</div>
              <div className="event-lineup">
                {event.artists.map((a, i) => (
                  <div key={a.artist.id} className="event-artist">
                    ...
                    {a.artist.resident && <span className="event-artist-tag">{dict.event.resident}</span>}
                  </div>
                ))}
              </div>
            </div>
          ...
          {event.description && (
            <div className="event-section">
              <div className="event-section-label">{dict.event.info}</div>
              <p className="event-description">{event.description}</p>
            </div>
          )}

          <div className="event-section">
            <div className="event-section-label">{dict.event.venue}</div>
            <div className="event-venue">
              ...
              {event.venue.capacity && (
                <div className="event-venue-cap">{dict.event.capacity}: {event.venue.capacity}</div>
              )}
            </div>
          </div>
          ...
          <div className="event-tickets">
            {hasTicket && (
              <div className="event-has-ticket">
                <div className="event-has-ticket-label">{dict.event.hasTicket}</div>
                <div className="event-has-ticket-count">
                  {userTickets.length} {userTickets.length === 1 ? dict.event.ticketSingle : dict.event.ticketPlural}
                </div>
                <Link href={`/${lang}/user/tickets`} className="back-link" style={{ marginTop: '12px' }}>
                  {dict.event.viewTickets}
                </Link>
              </div>
            )}
